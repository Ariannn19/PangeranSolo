...
void init_stack(stack *s) { // Fungsi untuk menginisialisasi stack
...
int is_empty(stack *s) { // Fungsi untuk memeriksa apakah stack kosong
    if (s->top == -1) {
...
void push(stack *s, char kertas_sekarang[100][100], int jumlah_baris) { // Fungsi untuk menambahkan keadaan kertas saat ini ke stack
    if (s->top >= 99) { // Cek apakah stack sudah penuh
...
void pop(stack *s, char kertas_tujuan[100][100], int *jumlah_baris_tujuan) { // Fungsi untuk mengambil keadaan kertas sebelumnya dari stack
    if (is_empty(s) == 1) {
...
void redo(stack *s, char kertas_tujuan[100][100], int *jumlah_baris_tujuan) { // Fungsi untuk melakukan redo
    if (s->top >= s->max_top) {  // Cek apakah sudah berada di posisi redo terakhir
